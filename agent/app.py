...
**[수정] 회복탄력성 지표 해석 (1~5점 스케일):**
- **회복탄력성지수 (평균 4.35):** 3개 하위 점수의 가중 평균. 낮을수록 위기 대응력이 약함.
- **재무적 탄력성 (평균 4.30):** 매출 안정성, 고정 고객 기반. 낮으면 현금 흐름이 불안정.
- **운영적 탄력성 (평균 3.98):** 신규 고객 유치, 성장성, 운영 안정성. 낮으면 성장 정체 또는 운영 비효율.
- **관계적 탄력성 (평균 4.95):** 고객 유지력, 단골 관리. 낮으면 고객 이탈률이 높음.
...
**[수정] 회복탄력성 지표 해석 (1~5점 스케일, 전체 3037개 매장 기준):**
- **회복탄력성지수 (평균 4.35):** 3개 하위 점수의 가중 평균. 낮을수록 위기 대응력이 약함.
    - (통계: 50%의 매장이 4.23 ~ 4.53점 사이에 분포. 최소 2.17점)
- **재무적 탄력성 (평균 4.30):** 매출 안정성, 고정 고객 기반. 낮으면 현금 흐름이 불안정.
    - (통계: 하위 25%를 제외한 75%의 매장이 4.33점 이상으로 편중되어 있음. 최소 1.0점)
- **운영적 탄력성 (평균 3.98):** 신규 고객 유치, 성장성, 운영 안정성. 낮으면 성장 정체.
    - (통계: 점수 편차가 가장 큼(std 0.59). 50%의 매장이 3.67 ~ 4.33점 사이에 분포. 최소 1.67점)
- **관계적 탄력성 (평균 4.95):** 고객 유지력, 단골 관리. 낮으면 고객 이탈률이 높음.
    - (통계: 대부분(75% 이상)의 매장이 5.0 만점. 이 점수가 낮다면 심각한 문제로 간주.)
- {{구체적 액션}} — {{기대효과/측정지표}}
...
# ================== Branch: 1) Store Analysis (Milvus RAG) ==================
    ...
    q = st.chat_input("가맹점 이름 또는 분석 포커스를 입력하세요 (예1: 동원* , 예2: 동원* 가맹점의 연령대 별 매출현황 요약)")
    if q:
