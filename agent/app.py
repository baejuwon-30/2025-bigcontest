...
**[수정] # 회복탄력성 기반 실행 제안 (우선순위 순, 3~5개)**
- **(지시) 컨텍스트에서 3가지 회복탄력성 점수(재무, 운영, 관계)를 확인하고, 이 중 **가장 낮은 점수(Weak Point)**에 초점을 맞춘 전략을 1순위로 제안하세요.**
- **(전략 예시) 재무 탄력성 낮을 시 → 정기 구독제, 예약 할인으로 고정 매출 확보**
- **(전략 예시) 운영 탄력성 낮을 시 → 메뉴 효율화, 유동인구 타겟 프로모션으로 신규 유입 증대**
- **(전략 예시) 관계 탄력성 낮을 시 → 단골 리워드 강화, 지역 연계 이벤트로 고객 락인(Lock-in)**
- {{구체적 액션}} — {{기대효과/측정지표}}
